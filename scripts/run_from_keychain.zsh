...
while IFS='=' read -r key value; do
  ...
  [[ -z "$key" || "$key" == \#* ]] && continue
  case "$key" in
    DISCORD_GUILD_IDS|MONITOR_ALL_GUILDS|MONITORED_CHANNEL_IDS|ALERT_CHANNEL_IDS|VOICE_ENABLED|VOICE_AUTO_JOIN|VOICE_CHANNEL_IDS|VOICE_ALERT_CHANNEL_IDS|MODERATION_BACKEND|GROQ_TEXT_MODEL|GROQ_FALLBACK_TEXT_MODEL|GROQ_VOICE_TEXT_MODEL|GROQ_SPEECH_MODEL|GROQ_FALLBACK_SPEECH_MODEL|GROQ_CONFIDENCE_THRESHOLD|GROQ_TIMEOUT_SECONDS|GROQ_TEXT_INTERVAL_SECONDS|GROQ_VOICE_INTERVAL_SECONDS|GROQ_AUDIO_INTERVAL_SECONDS|VOICE_CHUNK_SECONDS|CONTEXT_MESSAGE_COUNT|CONTEXT_TTL_SECONDS|VOICE_MIN_RMS|VOICE_MIN_UTTERANCE_MS)
      export "$key=$value"
      ;;
  esac
done < "$CONFIG_FILE"
