...
無料枠を通常会話で使い切らないよう、まず端末内ルールで一次判定します。明白な違反はその場で通知し、属性語・侮辱・嘲笑・隠語などの部分的な兆候だけがある曖昧な発言をGPT-OSSへ送ります。外部テキスト判定は4秒間隔、うちVC由来は45秒間隔とし、1日あたりテキスト70件・VC30件を上限にします。429時はGroqの待機時間（取得できない場合60秒）だけ外部判定を休止し、再試行の連打や処理待ちの蓄積を防ぎます。
