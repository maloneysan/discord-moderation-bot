...
任意でVCも監視できます。VC音声はメモリ内で短いWAVへ変換し、通常はGroq Whisper Large V3 Turbo、予備はWhisper Large V3、API枠が埋まっている場合は待ち行列を作らず端末内Voskで文字起こしして同じ判定へ渡します。通知には発言者の表示名、該当ワード／発言（最大240文字）、問題だった理由を記載します。本文・音声・文字起こしはBotのファイルやDB・ログへ保存しません。詳しい動作と制約は[仕様書](docs/SPECIFICATION.md)を参照してください。
...
音声APIの無料枠切れにもVC認識を継続する場合は、日本語Voskモデルを一度だけ取得します。

```bash
.venv/bin/python scripts/download_vosk_model.py
```

モデルは`models/`へ配置され、Gitには追加されません。音声も文字起こしも端末へ保存せず、メモリ内で処理します。
...
export GROQ_SPEECH_MODEL='whisper-large-v3-turbo'
export GROQ_FALLBACK_SPEECH_MODEL='whisper-large-v3'
export GROQ_CONFIDENCE_THRESHOLD='25'
export GROQ_TEXT_INTERVAL_SECONDS='10'
export GROQ_VOICE_INTERVAL_SECONDS='10'
export GROQ_AUDIO_INTERVAL_SECONDS='6'
...
export VOICE_MIN_RMS='25'
export VOICE_MIN_UTTERANCE_MS='100'
```
...
BotはVC参加時、VC内テキストまたは通知先へ音声認識中であることと非保存方針を案内します。発話は最大10秒、Discordの発話終了イベント、または音声後の無音検出時に送信します。高感度モードでは小さい声と短い発話を拾うためRMS閾値を25、最短発話を100msへ調整しています。Whisper Large V3 Turboへ日本語を明示し、`verbose_json`のうち`avg_logprob < -1.4`または`no_speech_prob > 0.92`だけを低品質として除外します。外部音声認識は6秒に1件までとし、枠内の即時1件に加えて次の1件だけを最大6秒間メモリ内で待機させます。それ以上の音声は待ち行列へ積まずVoskへ渡します。Turboが429のときだけLarge V3を試し、両方が利用不能ならVoskへ退避します。音声文字起こしだけに同音誤認識補正を適用しますが、認識プロンプトへ検知語を列挙せず、雑音が指定語へ補完される誤認を抑えます。高感度化により、従来より誤検知が増える可能性があります。[Groq公式の音声品質メタデータ解説](https://console.groq.com/docs/speech-to-text)

明確なローカルルール成立はAPIを待たず通知します。文脈判定は通常テキストをGPT-OSS 120B、VC文字起こしをモデレーション専用Safeguard 20Bへ最大10秒間隔で送り、429時はGPT-OSS 20Bへ退避します。枠外の要求はキューに積まずローカル判定へ戻るため、API失敗の連打と遅延の蓄積を防ぎます。[Groq公式レート制限](https://console.groq.com/docs/rate-limits)

`/vc_status`では認識済みチャンク、無音・低品質除外、最終認識時刻、直近エラーを確認できます。運用ログには文字起こし本文を残さず、チャンクが認識・判定まで到達したかだけを記録します。

VC通知には発言者のサーバー表示名、音声認識した該当ワード／発言（最大240文字）、問題点の具体的な説明を載せます。改行、メンション、Markdown記号は無効化し、音声や文字起こしをBotのDB・ファイル・ログへ保存しません。通知先未指定時は、Botが送信できるシステムチャンネルまたは最初のテキストチャンネルを自動選択します。20秒以内の同一話者・同一分類は1回へまとめます。
...
- `threshold`: 通知する最低点（高感度モードの初期値55）
- `categories`: 内部分類と通知ラベル
- `exceptions`: 批判的な引用や反差別表現などの除外正規表現

ローカル安全網は、日本語・英語の明示的な蔑称、より多くの国籍・民族・宗教・性自認・障害・疾病・生活状況、排除・権利否定・非人間化、直接的な暴言と煽り、性的表現、薬物名を追加で扱います。「草」「葉っぱ」「アイス」「スピード」「ネタ」のような一般語は単独では薬物扱いせず、摂取・売買・入手を示す語との組み合わせだけを対象にします。空白、中点、ピリオド、スラッシュ、縦線、アスタリスク、ハイフンによる難読化も正規化して照合します。
- `rules`: ルールID、分類、点数、正規表現

GPT-OSS 120Bは現在メッセージ、返信元、直前3件の一時文脈から、保護属性、脆弱集団、隠語、婉曲表現、集団責任、劣等視、排除、権利否定、非人間化、侮辱、見下し、嘲笑などを多言語で判定します。「女々しい」「男らしくない」「男のくせに泣くな」などの性別に基づく侮蔑は差別と冷笑の両方として扱います。発言スタイルから「女性らしい」などを推測するのではなく、明示的な性別侮蔑表現だけがローカル強ルールの対象です。
...
成立した分類ごとに、GPT-OSS 120Bが問題だった意味や会話上の作用を日本語1文で要約します。既知語は内部ルールIDから「『うお』はサーバー指定の冷笑反応」「『女々しい』は性別役割を押しつける侮蔑」など具体的に説明します。通知にはこれとは別に実際の該当ワード／発言を最大240文字で表示し、改行・メンション・Markdown記号を無効化します。
...
[config/evaluation_cases.json](config/evaluation_cases.json)には、実投稿でなく79件の人工例文を収録しています。明白な違反、婉曲・伏字、複数投稿文脈、複合分類、日本語・英語の蔑称と暴言、高感度モードの単独侮辱、性別侮蔑、下ネタ、ADHD、薬物名・乱用・売買・隠語、サーバー固有語、引用・反差別・中立言及・一般的医療を同じ基準で測れます。結果には本文を出さず、集計値と不一致ケースIDだけを表示します。
...
現在のローカル安全網は79件で適合率1.000、再現率0.932、F1 0.965です。ローカルで不一致となる4件は、複数投稿や婉曲表現など外部AIの文脈判定を前提にした例です。
...
7. 通知に発言者の表示名、該当ワード／発言（最大240文字）、具体的な問題点が載り、メンションは発生しない。
...
- Discord通知には発言者の現在の表示名、該当ワード／発言（最大240文字）、具体的な問題点が残ります。問題表現の再掲や表示名を公開したくないサーバーでは、`/alert_channel`で管理者専用チャンネルを指定してください。
- 無料枠やモデル提供状況は変更される場合があります。音声APIが利用不能な間は端末内Voskへ退避しますが、短語・雑音・重なり発話には聞き間違いが残ります。
