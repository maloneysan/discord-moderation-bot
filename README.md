...
テキストチャットは端末内ルールで一次判定し、曖昧な候補を外部APIへ送ります。VC文字起こしは登録語がなくても専用の`openai/gpt-oss-safeguard-20b`へ送り、複雑な言い回しも扱います。明白なローカル違反はその場で通知します。Bot独自の日次件数上限は設けず、外部判定はGroqのモデル別無料枠まで使用します。レート制限を避けるため外部判定は全体4秒間隔、VC由来は5秒間隔とし、VCモデルを通常テキスト用120B/20Bから分離します。Groq側の上限・429・通信障害中は、通常80点より少し広い65点のローカル退避判定へ自動移行し、「対象＋直接侮辱」「属性＋排除・固定観念」「失敗＋嘲笑」の組み合わせを継続検知します。属性語・笑い・軽い反応の単独出現は成立しません。
...
- Groq無料枠にはリクエスト数だけでなくモデル別の1日・1分トークン上限と音声時間上限があります。Botは一次判定と日次回数制御で消費を抑えますが、上限、モデル変更、サービス停止により外部判定できない場合があります。その間も明白なローカル表現とVosk音声認識は継続します。
